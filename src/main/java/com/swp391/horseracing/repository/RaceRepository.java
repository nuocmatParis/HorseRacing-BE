...
public interface RaceRepository extends JpaRepository<Race, UUID> {
    ...
    @Query("""
            SELECT DISTINCT r FROM Race r
            WHERE r.schedulePublishedAt IS NOT NULL
              AND (:fromTime IS NULL OR r.startTime >= :fromTime)
            ...
            ORDER BY r.startTime ASC
            """)
    Page<Race> findParticipantRacesForOwner(@Param("userId") UUID userId,
    ...
    @Query("""
            ...
            ORDER BY r.startTime ASC
            """)
    Page<Race> findParticipantRacesForJockey(@Param("userId") UUID userId,
                                             @Param("fromTime") LocalDateTime fromTime,
                                             @Param("toTime") LocalDateTime toTime,
                                             Pageable pageable);
